-- Script completo para corregir la lógica de disponibilidad en todas las funciones
-- Un día se considera disponible si hay al menos un bloque de 30 minutos disponible entre las 8:00 y las 19:00

-- 1. Corregir la función obtener_disponibilidad_sala_mes_v2
...
BEGIN
  ...
  FOR v_dia IN SELECT generate_series(v_fecha_inicio, v_fecha_fin, '1 day'::interval)::DATE LOOP
    ...
    fecha := v_dia;
    disponible := v_bloques_disponibles > 0; -- Al menos un bloque de 30 minutos disponible
...
-- 2. Corregir la función obtener_disponibilidad_sala_mes_json
...
-- 3. Corregir la función obtener_disponibilidad_sala_mes_json_v2
...
BEGIN
  ...
  FOR v_dia IN SELECT generate_series(v_fecha_inicio, v_fecha_fin, '1 day'::interval)::DATE LOOP
    ...
    v_disponible := v_bloques_disponibles > 0;
    ...
    v_dias_array := v_dias_array || jsonb_build_object(
      ...
      'porcentaje_disponibilidad', 
        CASE 
          WHEN v_bloques_totales > 0 
          THEN ROUND((v_bloques_disponibles::NUMERIC / v_bloques_totales) * 100, 2)
          ELSE 0
        END
    );
  END LOOP;
